...
class CMB2_Types {
	...
	public function maybe_custom_field_object( $fieldtype ) {
		...
		if ( $render_class_name && class_exists( $render_class_name ) ) {
			...
			if ( ! ( $this->type instanceof CMB2_Type_Base ) ) {
				throw new Exception( __( 'Custom CMB2 field type classes must extend CMB2_Type_Base.', 'distinctpress-pro' ) );
			}
		}
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<button type="button" data-selector="<?php echo $table_id; ?>" class="cmb-add-row-button button"><?php echo esc_html( $this->_text( 'add_row_text', esc_html__( 'Add Row', 'distinctpress-pro' ) ) ); ?></button>
		</p>
	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo $class; ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button type="button" class="button cmb-remove-row-button<?php echo $disabled; ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'distinctpress-pro' ) ) ); ?></button>
			</div>
		</div>
