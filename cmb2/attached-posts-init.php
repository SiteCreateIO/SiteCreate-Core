...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		if ( ! $query_users ) {
			...
			$post_type_labels = $field_type->_text( 'users_text', esc_html__( 'Users' , 'distinctpress-pro') );
		}

		// Check 'filter' setting
		$filter_boxes = $field->options( 'filter_boxes' )
			? '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Filter %s', 'distinctpress-pro' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>'
			: '';
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Available %s', 'distinctpress-pro' ), $post_type_labels ) . '</h4>';
		...
		foreach ( $objects as $object ) {
			...
			// Build our list item
			printf(
				'<li data-id="%d" class="%s">%s<a title="' . __( 'Edit', 'distinctpress-pro' ) . '" href="%s">%s</a><span class="dashicons dashicons-plus add-remove"></span></li>',
				$object->ID,
				$class,
				$thumbnail,
				$edit_link,
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Attached %s', 'distinctpress-pro' ), $post_type_labels ) . '</h4>';
		...
		// HMMMMM, DUBIOUS
		echo stripslashes( addslashes($field_type->input( array(
			'type'  => 'hidden',
	...
	protected function display_attached( $field, $attached ) {
		...
		foreach ( $attached as $id ) {
			...
			// Build our list item
			printf(
				'<li data-id="%d" class="%s">%s<a title="' . __( 'Edit', 'distinctpress-pro' ) . '" href="%s">%s</a><span class="dashicons dashicons-minus add-remove"></span></li>',
				$id,
				$class,
				$thumbnail,
				$edit_link,
