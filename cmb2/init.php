...
if ( ! class_exists( 'CMB2_Bootstrap_224_Trunk', false ) ) {
	...
	class CMB2_Bootstrap_224_Trunk {
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'distinctpress-pro', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'distinctpress-pro', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'distinctpress-pro', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'distinctpress-pro' );
				$mofile = dirname( __FILE__ ) . '/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'distinctpress-pro', $mofile );
			}
