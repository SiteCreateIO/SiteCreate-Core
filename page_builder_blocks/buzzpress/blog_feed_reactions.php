...
/* ADD OUR BLOCKS */
add_action('init', 'distinctpress_blog_feed_reactions_block', 99 );

// BLOG FEED
function distinctpress_blog_feed_reactions_block() { 
  if (function_exists('kc_add_map')) { 
      kc_add_map(
          array( 
              'blog_feed_reactions_layout' => array(
                  'name' => __('Blog Feed Reactions', 'distinctpress'),
...
function blog_feed_reactions_layout_shortcode($atts, $content = null){
    ...
    $args = array(
        ...
        'orderby'   => 'meta_value',
        'meta_key' => 'dw_reaction_total_liked',
    );
    ...
    $blog_reactions_query = new WP_Query ( $args );

    ob_start(); ?>

    <div class="clear-me row">

    <?php if ( $blog_reactions_query -> have_posts() ) :
        while ( $blog_reactions_query -> have_posts() ) : $blog_reactions_query -> the_post();
...
add_shortcode('blog_feed_reactions_layout', 'blog_feed_reactions_layout_shortcode'); 
